...
export function InvoiceForm({ settings, customers, products, onSave }: InvoiceFormProps) {
  ...
  const totals = useMemo(() => {
    ...
    const status = getInvoiceStatus(form.status, form.dueDate, result.amountPaid, result.balanceDue);
    return { ...result, status };
  }, [form]);

  function updateField<K extends keyof InvoiceFormState>(key: K, value: InvoiceFormState[K]) {
        ...
        <Card>
          ...
          <label className="field">
            ...
            <select value={form.customerId || ""} onChange={(event) => selectCustomer(event.target.value)}>
              <option value="">Custom / New customer</option>
